	...
	if os.path.exists(PACKAGES):
		try:	
			for root, dirs, files in os.walk(PACKAGES):
				...
				if file_count > 0:
					if over: yes=1
					else: yes=DIALOG.yesno("Borrar Packages!?", str(file_count) + " files found", "Do you want to delete them?", nolabel='No, Cancel',yeslabel='Yes, Remove')
					if yes:
						for f in files:	os.unlink(os.path.join(root, f))
						for d in dirs: shutil.rmtree(os.path.join(root, d))
						LogNotify(ADDONTITLE,'[COLOR lime]Borrando Packages:[/COLOR] [COLOR gold]Borrado[/COLOR]!')
				else: LogNotify(ADDONTITLE,'[COLOR orangered]Packages:[/COLOR] [COLOR white]No Se Han Encontrado[/COLOR]')
		except: LogNotify(ADDONTITLE,'[COLOR red]Packages:[/COLOR] [COLOR red]Error[/COLOR]!')
	else: LogNotify(ADDONTITLE,'[COLOR orangered]Packages:[/COLOR] [COLOR gold]No Se Han Encontrado[/COLOR]')
				...
				for d in dirs:
					if 'cache' in d.lower():
						try:
							shutil.rmtree(os.path.join(root, d))
							delfiles += 1
							log("[COLOR gold][Success] Wiped %s [/COLOR]" % os.path.join(item,d))
						except:
							log("[COLOR red][Failed] To Wipe Cache In: %s [/COLOR]" % os.path.join(item,d))

	LogNotify(ADDONTITLE,'[COLOR orangered]Borrando Cache:[/COLOR] [COLOR white] %s Items Removed[/COLOR]' % delfiles)
